  ...
  return (
    <div className="relative min-h-screen bg-cream text-charcoal font-sans selection:bg-terracotta/20">
      ...
      <nav className="fixed top-0 left-0 w-full z-50 px-6 md:px-12 py-6 flex justify-between items-center transition-all duration-500">
        <Link to="/" className="text-3xl font-display italic font-bold text-charcoal link-underline z-50 relative">
          A.V.
        </Link>
        ...
        <div className="hidden md:flex items-center space-x-12 uppercase tracking-[0.3em] text-[10px] font-bold z-50">
           {isCaseStudy ? (
             <Link to="/work" className="group flex items-center space-x-4">
             ...
             <>
                <Link to="/work" className={`link-underline hover:text-terracotta transition-colors ${location.pathname === '/work' ? 'text-terracotta' : ''}`}>Work</Link>
                <Link to="/about" className={`link-underline hover:text-terracotta transition-colors ${location.pathname === '/about' ? 'text-terracotta' : ''}`}>About</Link>
                <Link to="/contact" className={`link-underline hover:text-terracotta transition-colors ${location.pathname === '/contact' ? 'text-terracotta' : ''}`}>Contact</Link>
             </>
           )}
        </div>
        ...
        <footer className="relative z-10 py-12 border-t border-charcoal/5 px-6 md:px-12 lg:px-24 bg-cream">
          <div className="max-w-screen-2xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8 text-[10px] font-bold uppercase tracking-[0.4em]">
            <div className="text-charcoal flex items-center gap-4">
              <span className="text-xl italic font-display lowercase tracking-normal">a.v.</span>
              <span>Editorial Heritage</span>
            </div>
            <div className="flex gap-8 md:gap-12">
              <a href="#" className="link-underline hover:text-terracotta transition-colors">Instagram</a>
              <a href="#" className="link-underline hover:text-terracotta transition-colors">LinkedIn</a>
              <Link to="/work" className="link-underline hover:text-terracotta transition-colors">Portfolio</Link>
            </div>
            <div className="text-charcoal/40">© 2024 • All Rights Reserved</div>
          </div>
        </footer>
      )}
