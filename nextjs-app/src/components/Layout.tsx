...
function Wordmark({ className = "", invert = false }: { className?: string; invert?: boolean }) {
  return (
    <span className={`wm ${invert ? "wm--invert" : ""} ${className}`}>
      <span className="first">Claire</span>
      <span className="second">Webb</span>
    ...
    <div className="d2 relative min-h-screen">
      ...
      <header className="sticky top-0 z-50 bg-paper border-b border-[var(--rule)] py-[22px]">
        <div className="container grid grid-cols-[auto_1fr_auto] items-center gap-10">
          <Link href="/" className="relative" aria-label="Claire Webb — home">
            <Wordmark className="text-[28px]" />
          </Link>

          {/* Desktop nav */}
          <nav className="hidden md:flex gap-7 justify-center fr-label text-[12px] tracking-[0.18em] uppercase">
            {isCaseStudy ? (
          ...
          <Link
            href="/contact"
            className="hidden md:inline-flex items-center gap-2.5 fr-label text-[12px] tracking-[0.18em] uppercase text-ink border-b border-ink pb-1 hover:text-azure hover:border-azure transition-colors"
          >
          ...
          <button
            ...
            className="md:hidden justify-self-end relative text-ink hover:text-azure transition-colors"
        ...
        <div className="border-b border-[var(--rule)] py-3.5 fr-label text-[11px] tracking-[0.2em] uppercase text-ink-2">
          <div className="container flex justify-between items-center gap-6">
            ...
            <div className="flex items-center gap-3 text-azure">
              <svg width="18" height="18" viewBox="0 0 100 100" fill="none" stroke="currentColor" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round">
                <circle cx="50" cy="50" r="40" />
                <path d="M50 26 L 60 50 L 50 74 L 40 50 Z" fill="currentColor" stroke="none" />
              </svg>
        ...
        <footer className="bg-ink text-paper pt-20 pb-8">
          <div className="container">
            <div className="pb-14 border-b border-[rgba(246,247,243,0.18)] text-center">
              <Link href="/" aria-label="Claire Webb — home">
                <Wordmark invert className="text-[clamp(48px,9vw,76px)]" />
              </Link>
            ...
            <div className="pt-14 grid grid-cols-2 md:grid-cols-[1.5fr_1fr_1fr_1fr] gap-12">
              <p className="col-span-2 md:col-span-1 italic fr-subhead text-[23px] leading-[1.3] tracking-[-0.01em] text-paper max-w-[400px] m-0">
                ...
                <span className="not-italic text-lemon fr-display">No buzzwords. No &ldquo;wanderlust vibes&rdquo;.</span>{" "}
      ...
      <div className="fixed bottom-8 right-8 z-40">
        <button
          onClick={scrollToTop}
          className="w-12 h-12 border border-[var(--rule-strong)] bg-paper/90 backdrop-blur-sm text-ink flex items-center justify-center hover:bg-azure hover:text-white hover:border-azure transition-all group"
