...
export default async function WebsiteCopy() {
  const data = await reader.singletons.websiteCopy.read();

  const subtitleLabel = data?.subtitleLabel ?? "Digital Narratives";
  const description =
    data?.description ??
  ...
  const quote = data?.quote ?? "User experience is written as much as it is designed.";
  const ctaHeading = data?.ctaHeading ?? "Optimize";
  const ctaSubheading = data?.ctaSubheading ?? "your impact.";
  const ctaLinkText = data?.ctaLinkText ?? "Inquire about Web Copy";
      ...
      <header className="mb-24 relative">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-baseline gap-4 md:gap-12">
          <h1 className="text-6xl md:text-9xl font-display text-charcoal tracking-tighter leading-none">
            Website <br />
            <span className="italic font-normal text-terracotta">Copy.</span>
          </h1>
